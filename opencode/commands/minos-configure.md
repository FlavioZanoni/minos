...
- Ship new `llm-judge` rules as `"action": "warn"` first; tell the user to promote to `block` once the false-positive rate looks fine (the Minos config UI has a test sandbox for this).
...
```
echo '{"event":"command","tool":"Bash","command":"<sample>","cwd":"<project root>"}' | npx -y github:FlavioZanoni/minos check
```
