...
export async function anyRegexMatchesBounded(
  patterns: string[],
  flags: string,
  haystack: string,
  timeoutMs = 1000,
): Promise<{ fired: boolean; timedOut: boolean }> {
  const w = new Worker(WORKER_SRC, { eval: true });
  try {
    return await new Promise((resolve) => {
      let timer: NodeJS.Timeout;
      const done = (r: { fired: boolean; timedOut: boolean }) => {
        clearTimeout(timer);
        resolve(r);
      };
      timer = setTimeout(() => done({ fired: false, timedOut: true }), timeoutMs);
      w.once('message', (fired: boolean) => done({ fired: Boolean(fired), timedOut: false }));
