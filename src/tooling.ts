...
const DEFAULT_DESCRIBE: NonNullable<ToolingConfig['describe']> = [
  'declared',
  'tldr',
  'help',
  'man',
...
async function npmScriptsLines(cwd: string): Promise<string[]> {
...
async function makefileLines(cwd: string): Promise<string[]> {
...
async function justfileLines(cwd: string): Promise<string[]> {
  let text = await readFileMaybe(path.join(cwd, 'justfile'));
  if (!text) text = await readFileMaybe(path.join(cwd, 'Justfile'));
  if (!text) return [];
  const lines: string[] = [];
  for (const raw of text.split('\n')) {
    if (/^\s/.test(raw) || raw.trim() === '' || raw.startsWith('#')) continue;
    const m = raw.match(/^([A-Za-z_][A-Za-z0-9_-]*)/);
    if (!m) continue;
    const rest = raw.slice(m[0].length);
    if (!rest.includes(':')) continue;
    if (rest.trimStart().startsWith(':=')) continue; // variable assignment
    lines.push(`just ${m[1]}`);
  }
  return lines;
}
...
async function binEntries(cwd: string): Promise<BinEntry[]> {
...
async function tldrDescribe(name: string): Promise<string | undefined> {
  try {
...
async function helpDescribe(binPath: string): Promise<string | undefined> {
  try {
...
async function manDescribe(name: string): Promise<string | undefined> {
  try {
...
async function describeBinEntry(
  entry: BinEntry,
  order: string[],
...
async function buildToolingContext(cwd: string, tooling: ToolingConfig): Promise<string> {
  ...
  if (sources.includes('bin/*')) {
    ...
    for (const entry of entries) {
      if (declaredByName.has(entry.name)) continue; // already listed above
      const desc = await describeBinEntry(entry, order, declaredByName);
      if (desc) lines.push(`${entry.name} - ${desc}`);
...
export async function resolveToolingContext(
  cwd: string,
