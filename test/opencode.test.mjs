...
import { RuleGuardPlugin } from '../dist/opencode.js';

function fixture() {
  const dir = mkdtempSync(join(tmpdir(), 'rg-oc-'));
  mkdirSync(join(dir, '.rule-guard'));
  writeFileSync(join(dir, '.rule-guard', 'rules.jsonc'), JSON.stringify({
    rules: [
...
test('opencode adapter', async (t) => {
  ...
  const hooks = await RuleGuardPlugin({ directory: dir });
  ...
  await t.test('warn appends to tool output', async () => {
    ...
    assert.match(out.output, /rule-guard:todo-warn/);
  });
